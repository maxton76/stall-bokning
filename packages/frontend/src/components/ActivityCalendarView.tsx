    ...
    if (viewMode === "horses") {
      ...
      if (selectedHorseId !== "all") {
        filtered = filtered.filter(
          (a) => a.type === "activity" && a.horseId === selectedHorseId,
        );
      }
      return filtered;
  ...
  const transformActivity = useCallback(
    (entry: ActivityEntry): CalendarEvent => {
      ...
      if (entry.type === "activity") {
        ...
        const typeColor =
          entry.activityTypeColor || getActivityTypeColor(entry);
        return {
          ...baseEvent,
        ...
        return {
          ...baseEvent,
          title: entry.assignedToName
            ? `${entry.assignedToName}: ${entry.title}`
            : entry.title,
