...
import { FormInput, FormTextarea, FormDatePicker, FormSelect } from '@/components/form'
import { Alert, AlertDescription } from '@/components/ui/alert'
import { Info } from 'lucide-react'
import { getAllAvailableVaccinationRules } from '@/services/vaccinationRuleService'
import { createVaccinationRecord, updateVaccinationRecord } from '@/services/vaccinationService'
...
export function VaccinationRecordDialog({
  ...
  const [availableRules, setAvailableRules] = useState<VaccinationRule[]>([])
  const [loadingRules, setLoadingRules] = useState(true)
  const [autoCalculatedDueDate, setAutoCalculatedDueDate] = useState<string>('')
  ...
  useEffect(() => {
    const fetchRules = async () => {
      try {
        ...
        const rules = await getAllAvailableVaccinationRules(
          horse.ownerId,
          organizationId
        )
        setAvailableRules(rules)
      } catch (error) {
        console.error('Failed to fetch vaccination rules:', error)
      } finally {
  ...
  }, [open, horse.ownerId, organizationId])

  const { form, handleSubmit, resetForm } = useFormDialog<VaccinationRecordFormData>({
    ...
    onSubmit: async (data) => {
      const vaccinationDate = Timestamp.fromDate(new Date(data.vaccinationDate))
      const nextDueDate = Timestamp.fromDate(new Date(data.nextDueDate))

      const selectedRule = availableRules.find(r => r.id === data.vaccinationRuleId)
      ...
      if (isEditMode && record) {
        ...
        if (!user) {
          throw new Error('User must be authenticated to create vaccination record')
        }
  ...
  useEffect(() => {
    if (vaccinationRuleId && vaccinationDate) {
      const selectedRule = availableRules.find(r => r.id === vaccinationRuleId)
      if (selectedRule) {
        ...
        if (!form.getValues('nextDueDate') || form.getValues('nextDueDate') === '') {
  ...
  const dialogTitle = title || (isEditMode ? 'Edit Vaccination Record' : 'Add Vaccination Record')
    ...
    >
      <div className="space-y-4">
        ...
        <FormSelect
          ...
          form={form}
          options={ruleOptions}
          placeholder={loadingRules ? 'Loading rules...' : 'Select a vaccination rule'}
            ...
            <Alert>
              ...
              <AlertDescription className="text-sm">
                ...
                {form.getValues('nextDueDate') !== autoCalculatedDueDate && (
                  <span className="ml-1 text-muted-foreground">(manually overridden)</span>
                )}
              </AlertDescription>
            </Alert>
          )}
