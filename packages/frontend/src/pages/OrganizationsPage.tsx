...
export default function OrganizationsPage() {
  ...
  const { data: organizations, loading } = useAsyncData(
    async () => user ? await getUserOrganizations(user.uid) : [],
    [user]
  )
        ...
        <Card>
          <CardContent className='pt-6 text-center py-12'>
            ...
            <p className='text-sm text-muted-foreground mb-4'>
              Create your first organization to start managing multiple stables and team members
            </p>
            <Button asChild>
              <Link to='/organizations/create'>
                <Plus className='h-4 w-4 mr-2' />
                ...
                <div className='space-y-4'>
                  ...
                  <div className='grid grid-cols-2 gap-4 text-sm'>
                    <div>
                      ...
                      <p className='text-2xl font-bold'>{org.stats.stableCount}</p>
                    </div>
                    <div>
                      <p className='text-muted-foreground'>Members</p>
                      <p className='text-2xl font-bold'>{org.stats.totalMemberCount}</p>
                    </div>
                  </div>
