...
export default function ScheduleEditorPage() {
  ...
  // Fetch schedule
  const { data: schedule = null, isLoading: scheduleLoading } = useQuery({
  ...
  // Fetch stable members with user lookups
  const { data: members = [], isLoading: membersLoading } = useQuery({
    queryKey: ["stableMembers", "stable", stableId, "withUsers"],
    queryFn: async () => {
      if (!stableId) return [];
  ...
  const loading = scheduleLoading || shiftsLoading || membersLoading;
  ...
  const handleAutoAssign = async () => {
    ...
    const unassignedCount = shifts.filter(
    ...
    try {
      ...
      const historicalPoints = await calculateHistoricalPoints(
        stableId,
        memberIds,
        90,
      );
      ...
      alert(
        `Successfully auto-assigned ${assignedCount} shifts using the enhanced fairness algorithm!`,
      );
    } catch (error) {
  ...
  const stats = useMemo(() => {
    ...
    const totalPoints = Array.from(pointsByMember.values()).reduce(
      (sum, p) => sum + p,
      0,
    );
    const avgPoints =
      pointsByMember.size > 0 ? totalPoints / pointsByMember.size : 0;

    // Calculate fairness (lower standard deviation = more fair)
    const pointsArray = Array.from(pointsByMember.values());
    const variance =
      pointsArray.reduce((acc, p) => acc + Math.pow(p - avgPoints, 2), 0) /
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      {schedule.status === "draft" && stats.unassigned > 0 && (
        <Card className="bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800">
          ...
          <CardContent className="text-sm text-blue-700 dark:text-blue-300">
            <p>
              The auto-assignment algorithm distributes unassigned shifts fairly
              by:
            </p>
            <ul className="list-disc list-inside mt-2 space-y-1">
              <li>
                Calculating each member's current point total from assigned
                shifts
              </li>
              <li>
                Assigning each unassigned shift to the member with the lowest
                points
              </li>
              <li>
                Creating a balanced distribution that keeps everyone within ±10%
                of average
              </li>
            </ul>
            <p className="mt-2">
              <strong>Fairness Index:</strong> {stats.fairnessIndex}% - Higher
              scores indicate more balanced distribution
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Assign boarders to shifts. You can publish when ready, even with
            unassigned shifts.
            {members.length > 0 && (
