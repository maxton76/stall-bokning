...
export default function StableSchedulePage() {
  ...
  // Load shifts for this specific stable
  useEffect(() => {
    if (!user || !stableId) return
    loadStableShifts()
  }, [user, stableId])

  const loadStableShifts = async () => {
    if (!user || !stableId) return
    ...
    try {
      ...
      console.error('Error loading stable shifts:', error)
    } finally {
      setLoading(false)
    }
  }
          ...
          <CardContent className='space-y-4'>
            {upcomingUnassigned.length === 0 ? (
              ...
              upcomingUnassigned.map(shift => (
                <div key={shift.id} className='flex flex-col space-y-2 p-3 border rounded-lg'>
                  ...
                  <Button size='sm' className='w-full' onClick={() => handleAssignShift(shift)}>
                    Take Shift
                  </Button>
                </div>
              ))
                    ...
                    <div className='flex items-center gap-3'>
                      <div>
                        ...
                        <p className='text-sm text-muted-foreground'>
                          {shift.date.toDate().toLocaleDateString()} • {shift.time}
                        </p>
                      </div>
                    ...
                    {shift.status === 'assigned' ? (
                      <>
                        <div className='text-right'>
                          <p className='text-sm font-medium'>{shift.assignedToName}</p>
                          <p className='text-xs text-muted-foreground'>{shift.assignedToEmail}</p>
                        </div>
                      ...
                      <>
                        ...
                        <Button size='sm' onClick={() => handleAssignShift(shift)}>
                          Take Shift
                        </Button>
                      </>
                    )}
