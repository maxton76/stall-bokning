...
export default function SchedulePage() {
  ...
  const getShiftCoverageForDate = (date: Date) => {
    ...
    const shiftsOnDate = shifts.filter(s => s.date.toDate().toDateString() === dateStr)

    if (shiftsOnDate.length === 0) return { total: 0, assigned: 0, status: 'none' }
  ...
  const stats = useMemo(() => {
    ...
    const unassigned = stableFilteredShifts.filter(s => s.status === 'unassigned').length
    const assigned = stableFilteredShifts.filter(s => s.status === 'assigned').length
  ...
  const handleAssignShift = async (shift: Shift) => {
    ...
    try {
      await assign(shift.id, user.uid, user.email || 'Unknown', user.email || '')
      await loadAllShifts()
    } catch (error) {
      ...
      <div className='grid gap-6 md:grid-cols-3'>
        ...
        <Card className='md:col-span-2'>
          <CardHeader>
            ...
            <CardDescription>
              Days are color-coded: 🟢 All assigned • 🟡 Partially assigned • 🔴 Unassigned
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Calendar
              ...
              {upcomingUnassigned.length === 0 ? (
                ...
                upcomingUnassigned.map(shift => (
                  ...
                  >
                    <div className='flex items-start justify-between'>
                      <div>
                        <p className='font-medium text-sm'>{shift.shiftTypeName}</p>
                        <p className='text-xs text-muted-foreground'>{shift.stableName}</p>
                      </div>
                    ...
                    <div className='flex items-center gap-3 text-xs text-muted-foreground'>
                      <span className='flex items-center gap-1'>
                        ...
                        {shift.date.toDate().toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                      </span>
                      <span className='flex items-center gap-1'>
        ...
        <CardHeader>
          <div className='flex items-center justify-between'>
            <div>
              ...
              <CardDescription>View and manage all scheduled shifts</CardDescription>
            </div>
              ...
              {filteredShifts.map(shift => (
                ...
                >
                  <div className='flex-1'>
                    <div className='flex items-center gap-3'>
                      <div>
                        ...
                        <p className='text-sm text-muted-foreground'>{shift.stableName}</p>
                      </div>
                    </div>
                  </div>
                  <div className='flex items-center gap-4'>
                    ...
                    <div className='w-32'>
                      {shift.status === 'assigned' ? (
                        <Badge variant='outline' className='bg-green-50 text-green-700 border-green-200'>
                          {shift.assignedToEmail === user?.email ? 'You' : shift.assignedToName}
                        </Badge>
                      ) : (
                        <Badge variant='destructive'>Unassigned</Badge>
                      )}
                    ...
                    >
                      {shift.status === 'assigned' ? 'Unassign' : 'Assign to Me'}
                    </Button>
                  </div>
                </div>
              ))}
