...
export default function FacilitiesReservationsPage() {
  ...
  const filteredReservations = useMemo(() => {
    ...
    return reservations.data.filter(reservation => {
      ...
      if (selectedFacilityType !== 'all' && reservation.facilityType !== selectedFacilityType) {
        return false
      }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      upcoming: reservations.data.filter(r =>
        r.status === 'confirmed' && r.startTime.toDate() > now
      ).length,
      completed: reservations.data.filter(r =>
        r.status === 'completed'
      ).length
    }
  }, [reservations.data])
  ...
  const reservationsForSelectedDate = useMemo(() => {
    ...
    return filteredReservations.filter(reservation =>
      isSameDay(reservation.startTime.toDate(), selectedDate)
    )
  ...
  const datesWithReservations = useMemo(() => {
    ...
    return filteredReservations.map(r => r.startTime.toDate())
  ...
  const handleEventDrop = async (reservationId: string, newStart: Date, newEnd: Date, newFacilityId?: string) => {
    if (!user) return
  ...
  const handleEventResize = async (reservationId: string, newStart: Date, newEnd: Date) => {
    if (!user) return
  ...
  const handleSaveReservation = async (data: any) => {
    try {
      ...
      if (reservationDialog.data) {
        ...
        await updateReservation(reservationDialog.data.id, reservationData, user.uid)
        toast({
          title: 'Success',
  ...
  return (
    <div className='container mx-auto p-6 space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          <h1 className='text-3xl font-bold tracking-tight'>Facility Reservations</h1>
          <p className='text-muted-foreground mt-1'>View and manage facility bookings</p>
        </div>
        ...
        <>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Click a date to view reservations</CardDescription>
            </CardHeader>
          ...
          {selectedDate && (
            <Card>
              <CardHeader>
                <CardTitle>Reservations for {format(selectedDate, 'PPP')}</CardTitle>
                <CardDescription>
                      ...
                      >
                        <div className='flex-1'>
                          <div className='flex items-center gap-2'>
                            <h4 className='font-semibold'>{reservation.facilityName}</h4>
                            <Badge variant='outline'>
                              {FACILITY_TYPE_LABELS[reservation.facilityType]}
                            </Badge>
                          </div>
                          <p className='text-sm text-muted-foreground mt-1'>
                            {format(reservation.startTime.toDate(), 'HH:mm')} - {format(reservation.endTime.toDate(), 'HH:mm')}
                          </p>
