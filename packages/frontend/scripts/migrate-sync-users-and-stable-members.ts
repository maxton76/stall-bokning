...
function createUserFromMember(
  ...
  if (member.firstName && member.lastName) {
    ...
    stats.warnings.push(
      `Parsed name from email for ${member.userId}: ${member.userEmail} → ${firstName} ${lastName}`,
    );
    console.log(
      `  ⚙️  Parsed from email: ${member.userEmail} → ${firstName} ${lastName}`,
    );
  } else {
    firstName = "Unknown";
    lastName = "User";
    stats.warnings.push(
      `No name data available for ${member.userId}, using default`,
    );
    ...
    try {
      if (!user) {
        ...
        if (needsSync(user, member)) {
          ...
          console.log(
            `     Member: ${member.firstName || "N/A"} ${member.lastName || "N/A"}`,
          );
