...
export default async function organizationMemberRoutes(fastify: FastifyInstance) {

  // POST /api/v1/organization-members/:memberId/accept - Accept membership invite
  fastify.post('/:memberId/accept', {
    ...
    try {
      ...
      const [userId, organizationId] = memberId.split('_')
  ...
  fastify.post('/:memberId/decline', {
    ...
    try {
      ...
      // Parse memberId to get userId
      const [userId] = memberId.split('_')
