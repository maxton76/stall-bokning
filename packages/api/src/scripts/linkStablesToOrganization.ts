...
const STABLE_IDS = [
  '5lu8Irn4UMngdpFCcQ1d',
  'A2N94bFJry4devCSyO1F'
]
...
export async function linkStablesToOrganization() {
  ...
  for (const stableId of STABLE_IDS) {
    ...
    console.log(`      Current organizationId: ${stable?.organizationId || 'None'}`)
  }

  // 3. Check if migration is needed (idempotency)
  const alreadyLinked = stableData.filter(s => s.currentOrgId === ORGANIZATION_ID)
  const needsLinking = stableData.filter(s => s.currentOrgId !== ORGANIZATION_ID)
  ...
  for (const stable of needsLinking) {
    try {
      ...
      console.error(`   ❌ Error linking ${stable.name} (${stable.id}):`, error)
      errorCount++
    }
  }
  ...
  const orgStablesSnapshot = await db.collection('stables')
  ...
  try {
    ...
    console.log(`   ✅ Organization stats updated: stableCount = ${totalStableCount}`)
  } catch (error) {
  ...
  console.log('\n🔍 Verifying migration...')
  const verifyStablesSnapshot = await db.collection('stables')
    .where('organizationId', '==', ORGANIZATION_ID)
    .get()

  const verifyOrgDoc = await db.collection('organizations').doc(ORGANIZATION_ID).get()
  ...
  console.log(`   📊 Total stables in organization: ${verifyStablesSnapshot.size}`)
  console.log(`   📊 Organization stableCount: ${verifyOrg?.stats?.stableCount || 0}`)

  // Final validation
  if (verifyStablesSnapshot.size === STABLE_IDS.length &&
