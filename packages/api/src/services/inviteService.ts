...
export async function createOrganizationInvite(
  ...
  const invite: Omit<OrganizationInvite, 'id'> = {
    ...
    inviterName: `${inviterData?.firstName || ''} ${inviterData?.lastName || ''}`.trim()
  }
 ...
 */
export async function getInviteByToken(token: string): Promise<(OrganizationInvite & { id: string }) | null> {
 ...
 */
export async function migrateInvitesOnSignup(userId: string, email: string): Promise<void> {
