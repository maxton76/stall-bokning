...
async function queueNotification(
  ...
  const notificationRef = await db.collection("notifications").add({
    userId,
    userEmail,
    type: entityType.includes("health") ? "health_reminder" : "shift_reminder",
    priority,
    title,
    body,
    entityType,
 ...
 */
export const scanForReminders = onSchedule(
  {
  ...
  async (_event) => {
    ...
    try {
        ...
        try {
          ...
          for (const reminderMinutes of reminderTimes) {
            ...
            if (
              ...
              const wasReminderSent = await db.runTransaction(
                async (transaction) => {
      ...
      const horsesSnapshot = await db
        ...
        .where("nextVaccinationDue", "<=", Timestamp.fromDate(healthWindowEnd))
        .limit(BATCH_SIZE)
        .get();
      ...
      for (const instanceDoc of overdueSnapshot.docs) {
        try {
          ...
          if (instance.assignedTo) {
            ...
            const shouldSendNotification = await db.runTransaction(
              async (transaction) => {
