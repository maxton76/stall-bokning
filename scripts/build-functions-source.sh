...
# 6. Updates terraform.tfvars.local with the source object path
...
TFVARS_LOCAL="${TF_DIR}/terraform.tfvars.local"

# Create or update the local tfvars file
if [ -f "${TFVARS_LOCAL}" ]; then
  # Update existing values
  if grep -q "functions_source_bucket" "${TFVARS_LOCAL}"; then
    sed -i.bak "s|functions_source_bucket.*=.*|functions_source_bucket = \"${BUCKET}\"|" "${TFVARS_LOCAL}"
    rm -f "${TFVARS_LOCAL}.bak"
  else
    echo "functions_source_bucket = \"${BUCKET}\"" >> "${TFVARS_LOCAL}"
  fi

  if grep -q "functions_source_object" "${TFVARS_LOCAL}"; then
    sed -i.bak "s|functions_source_object.*=.*|functions_source_object = \"${ARCHIVE_NAME}\"|" "${TFVARS_LOCAL}"
    rm -f "${TFVARS_LOCAL}.bak"
  else
    echo "functions_source_object = \"${ARCHIVE_NAME}\"" >> "${TFVARS_LOCAL}"
  fi
else
